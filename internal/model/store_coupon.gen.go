...
type StoreCoupon struct {
	...
	IsLimited        bool                  `gorm:"column:is_limited;type:tinyint(1);comment:是否限量, 默认0 不限量， 1限量" json:"is_limited"`                         // 是否限量, 默认0 不限量， 1限量
	...
	UseType          int32                 `gorm:"column:use_type;type:tinyint;not null;default:1;comment:使用类型 1 全场通用, 2 商品券, 3 品类券" json:"use_type"`      // 使用类型 1 全场通用, 2 商品券, 3 品类券
	...
	IsFixedTime      bool                  `gorm:"column:is_fixed_time;type:tinyint(1);comment:是否固定使用时间, 默认0 否， 1是" json:"is_fixed_time"` // 是否固定使用时间, 默认0 否， 1是
	...
	Type             int32                 `gorm:"column:type;type:tinyint;not null;default:1;comment:优惠券类型 1 手动领取, 2 新人券, 3 赠送券" json:"type"` // 优惠券类型 1 手动领取, 2 新人券, 3 赠送券
	Sort             int64                 `gorm:"column:sort;type:int unsigned;not null;default:1;comment:排序" json:"sort"`                    // 排序
	Status           int32                 `gorm:"column:status;type:tinyint unsigned;not null;default:1;comment:状态（0：关闭，1：开启）" json:"status"` // 状态（0：关闭，1：开启）
	IsDel            int32                 `gorm:"column:is_del;type:tinyint unsigned;not null;comment:是否删除 状态（0：否，1：是）" json:"is_del"`        // 是否删除 状态（0：否，1：是）
