...
type StoreBargainUser struct {
	...
	Status          int32                 `gorm:"column:status;type:tinyint unsigned;not null;comment:状态 1参与中 2 活动结束参与失败 3活动结束参与成功" json:"status"` // 状态 1参与中 2 活动结束参与失败 3活动结束参与成功
	AddTime         int64                 `gorm:"column:add_time;type:bigint unsigned;comment:参与时间" json:"add_time"`                               // 参与时间
	IsDel           bool                  `gorm:"column:is_del;type:tinyint(1);not null;comment:是否取消" json:"is_del"`                               // 是否取消
