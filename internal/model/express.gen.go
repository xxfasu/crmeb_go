...
type Express struct {
	...
	PartnerID  bool                  `gorm:"column:partner_id;type:tinyint(1);not null;comment:是否需要月结账号" json:"partner_id"`      // 是否需要月结账号
	PartnerKey bool                  `gorm:"column:partner_key;type:tinyint(1);not null;comment:是否需要月结密码" json:"partner_key"`    // 是否需要月结密码
	Net        bool                  `gorm:"column:net;type:tinyint(1);not null;comment:是否需要取件网店" json:"net"`                    // 是否需要取件网店
	...
	Status     bool                  `gorm:"column:status;type:tinyint(1);not null;comment:是否可用" json:"status"`                  // 是否可用
